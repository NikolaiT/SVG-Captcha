...
function _approximate_line($line) {
    ...
    $d = sqrt(pow(abs($line[0]->x - $line[0]->x), 2) + pow(abs($line[0]->y - $line[0]->y), 2));
    ...
    $somewhere_near_the_line = function($line, $md) {
        ...
        $y = $m * $x + $d;
        echo "Random point chosen: ".(new Point($x, $y));
        // And move it away by $md :P
        return new Point((rand(0, 1) == 1) ? $md : -$md, (rand(0, 1) == 1) ? $md : -$md);
    };
